...
export function createChromeHarness({ tree = workspaceTree([]), persisted } = {}) {
  const local = persisted ? new Map(persisted.local) : new Map();
  const session = persisted ? new Map(persisted.session) : new Map();
  const nodes = new Map(); let sequence = persisted?.sequence ?? 10;
  ...
  const timers = new Map(), sockets = new Set(), queue = []; let phase = "after", duplicates = 1;
  const node = (id) => nodes.get(id);
  const materialize = (raw, parentId, index) => {
    const id = raw.id ?? String(sequence++); const record = { id, parentId, index, title: raw.title ?? "", url: raw.url, children: [] };
    nodes.set(id, record); (raw.children ?? []).forEach((child, childIndex) => record.children.push(materialize(child, id, childIndex))); return id;
  };
  const restore = (value) => { nodes.clear(); materialize(value, undefined, 0); };
  restore(persisted?.tree ?? tree);
  ...
  const complete = (kind, args, callback) => {
    const delivery = () => Array.from({ length: duplicates }, () => emit(kind, ...args));
    if (phase === "before") { delivery(); callback(); }
    else if (phase === "after") { callback(); delivery(); }
    else if (phase === "held") queue.push(() => { delivery(); callback(); });
    else { callback(); queue.push(delivery); }
  };
  const area = (store) => ({
  ...
  const bookmarks = {
    ...
    create(input, callback) { const parent = node(input.parentId ?? "2"); if (!parent) throw new Error("parent missing"); const id = String(sequence++), record = { id, parentId: parent.id, index: input.index ?? parent.children.length, title: input.title ?? "", url: input.url, children: [] }; parent.children.splice(Math.max(0, Math.min(record.index, parent.children.length)), 0, id); nodes.set(id, record); normalize(parent.id); complete("created", [id, view(id)], () => callback(view(id))); },
    update(id, changes, callback) { const record = node(id); Object.assign(record, changes); complete("changed", [id, changes], () => callback(view(id))); },
  ...
  const removeNode = (id, deep, callback) => { const record = node(id); if (!record || (!deep && record.children.length)) throw new Error("node missing or non-empty"); const parent = node(record.parentId); parent.children.splice(record.index, 1); normalize(parent.id); const erase = (current) => { node(current).children.forEach(erase); nodes.delete(current); }; erase(id); complete("removed", [id, { parentId: parent.id, index: record.index, node: viewFrom(record) }], callback); };
  ...
  const harness = { chrome: { runtime: { lastError: null, onMessage: message }, storage: { local: area(local), session: area(session) }, bookmarks }, WebSocket, fetch: { fetch, requests, respond: (value) => responses.push({ value }), reject: (error) => responses.push({ error }), defer: () => { let resolve, reject; const promise = new Promise((res, rej) => { resolve = res; reject = rej; }); responses.push({ deferred: true, promise }); return { resolve, reject }; }, mutationCount: () => requests.filter(({ method }) => !["GET", "HEAD"].includes(method)).length }, timers: { set: (fn) => { const id = Symbol(); timers.set(id, fn); return id; }, clear: (id) => timers.delete(id), flush: () => { for (const fn of timers.values()) fn(); timers.clear(); } }, mutators: { mode: (value, count = 1) => { phase = value; duplicates = count; }, flush: (order) => { const pending = queue.splice(0); (order ?? pending.map((_, index) => index)).forEach((index) => pending[index]?.()); }, settle: () => harness.mutators.flush(), pending: () => queue.length }, snapshot: () => clone({ local: [...local], session: [...session], tree: view("0", true), sequence }), openHandles: () => ({ listeners: events.reduce((total, item) => total + item.size, 0), timers: timers.size, sockets: sockets.size }), resetRuntime: () => { events.forEach((item) => item.clear()); timers.clear(); [...sockets].forEach((socket) => socket.close()); queue.length = 0; }, teardown: () => { harness.resetRuntime(); const handles = harness.openHandles(); if (Object.values(handles).some(Boolean)) throw new Error(`open handles: ${JSON.stringify(handles)}`); } };
  return harness;
}
