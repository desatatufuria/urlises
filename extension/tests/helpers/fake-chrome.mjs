...
export function createChromeHarness({ tree = workspaceTree([]), persisted } = {}) {
  ...
  const bookmarks = {
    ...
    move(id, destination, callback) { const record = node(id), oldParentId = record.parentId, oldIndex = record.index; const from = node(oldParentId), to = node(destination.parentId ?? oldParentId), sameParent = to.id === oldParentId; let index = destination.index ?? to.children.length; if (index < 0 || index > to.children.length) throw new Error("Index out of bounds."); if (sameParent && (index === oldIndex || index === oldIndex + 1)) { callback(view(id)); return; } if (sameParent && index > oldIndex) index -= 1; from.children.splice(oldIndex, 1); normalize(from.id); record.parentId = to.id; to.children.splice(index, 0, id); normalize(to.id); complete("moved", [id, { parentId: to.id, oldParentId, index: record.index, oldIndex }], () => callback(view(id))); },
