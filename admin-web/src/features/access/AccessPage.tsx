  ...
  return (
    <section className="ui-section-stack">
      <header className="ui-section-header">
        <h2 className="ui-section-title">Access</h2>
      ...
      {workspacesQuery.isPending ? <DataState title="Loading workspaces" description="Reviewing the available workspaces before grant assignment begins." /> : null}
      ...
      {!workspacesQuery.isPending && !workspacesQuery.isError && (workspacesQuery.data?.length ?? 0) === 0 ? (
        <DataState title="No workspaces available yet" description="Create the first workspace from the Workspaces route, then return here to add direct or group grants." />
      ) : null}
          ...
          <section className="ui-card ui-section-stack">
            <header className="ui-section-header">
              <div className="ui-actions-spread">
                <div>
                  <h3 className="ui-section-title">Workspace access targets</h3>
                  <p className="ui-copy">Pick one workspace to inspect raw grants and the effective-role result together.</p>
                </div>
                <Badge tone="neutral">{workspacesQuery.data?.length ?? 0} total</Badge>
              </div>
            </header>
              ...
              {workspacesQuery.data?.map((workspace) => (
                ...
                >
                  <div className="ui-cell-stack">
                    ...
                    <span className="ui-muted">{workspace.workspaceType}</span>
                  </div>
                  <Badge tone={workspace.workspaceId === selectedWorkspaceId ? "accent" : "neutral"}>{workspace.role}</Badge>
                </button>
              ))}
            </div>
          </section>
