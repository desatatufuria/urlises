...
Enable eligible URLises Control operators to create an additional organization and enter it immediately, without changing first-run setup or backend authorization.

## Requirements

### Requirement: Eligible Creation Discoverability

URLises Control MUST show an organization-creation shell entry point only after the authenticated user has a resolved `owner` or `admin` membership. The entry point MUST reach the dedicated creation page.
...
- GIVEN an authenticated URLises Control user without an owner or admin membership
- WHEN the shell is displayed
- THEN no organization-creation entry point is displayed

### Requirement: Protected Creation Route

The dedicated creation page MUST deny direct routed access to URLises Control users who are not eligible operators.
...
The form MUST submit the existing authenticated `POST /organizations` contract. On success, URLises Control MUST persist exactly the membership returned by that contract, select its organization as active, and navigate to that organization's overview before organization-scoped work resumes.
...
URLises Control MUST show definite API errors and permit corrected resubmission with a fresh idempotency key.
...
URLises Control MUST retain the same idempotency key for a same-intent retry after an uncertain transport failure, and MUST NOT treat that failure as a definite API error.
...
First-run setup MUST remain the only creation path for users with no organizations and MUST remain otherwise unchanged. This UI specification MUST NOT redefine existing backend authorization for `POST /organizations`. `OdA` remains workspace data and is outside the product-branding boundary.

#### Scenario: First-run user remains in setup

- GIVEN an authenticated user has no organizations
- WHEN the user enters URLises Control
...
- WHEN this URLises Control feature is present
