...
| `backend/internal/organizations/service.go` | Modify | 2, 4 | `CancelInvitation`, `DeleteOrganization`, `ErrWouldOrphanMember`, orphan probe helper |
...
4. **Cancelled invitations disappear from the list rather than showing `cancelled` (slice 2).** The spec scenario says the list "refreshes to reflect `cancelled` status", but `ListInvitations` filters `status = 'pending'` (`service.go:616`). The row vanishes on refetch. Widening that query would change an existing endpoint's response and the empty-state copy. Confirm "disappears" satisfies the scenario, or accept the scope increase.

## Testing Strategy

| Layer | What to test | Approach |
|---|---|---|
| Unit — `CancelInvitation` | admin gate; `pending` → `cancelled` + activity row committed atomically; already-`cancelled`/`accepted`/`expired` → `ErrInvitationNotPending` with no activity row; unknown ID → `ErrNotFound` | table-driven against the test DB, extending existing `organizations` service tests |
