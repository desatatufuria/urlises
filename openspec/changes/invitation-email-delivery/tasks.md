...
- [x] 5.1 `admin-web/src/lib/api/types.ts` — add `AcceptedInvitation { organizationId, organizationName, role: OrganizationRole }` alongside `PendingInvitation`.
- [x] 5.2 Create `admin-web/src/lib/api/invitations.ts` — `acceptInvitation(accessToken, invitationToken)` calling `POST /invitations/{token}/accept` via `apiRequest`, kept separate from `organizations.ts` per the design's module-ownership decision.

## Phase 6: Frontend Routing and Pages

- [x] 6.1 RED: `admin-web/src/app/router.test.tsx` — assert `/invitations/:token` resolves to a public route and is not swallowed by the `*` catch-all.
- [x] 6.2 GREEN: `admin-web/src/app/router.tsx` — register `{ path: "/invitations/:token", element: <InvitationAcceptPage /> }` as a top-level route outside `RequireSession`.
...
- [x] 6.4 RED (threat-matrix: open redirect via `returnTo`) — same file or `router.test.tsx`: assert `returnTo`/post-auth navigation never resolves to an external origin, only to the fixed `/invitations/:token` local path.
- [x] 6.5 GREEN: create `admin-web/src/app/views/InvitationAcceptPage.tsx` — `useParams`/`useSearchParams`, branch on `useAuth().status`, `useRef`-guarded single-flight `acceptInvitation` call on `authenticated`, outcome screen ("joined {org} as {role}", console link only for `owner`/`admin`), and the four inline error mappings from the design.
- [x] 6.6 `admin-web/src/app/views/LoginPage.tsx` — add `useSearchParams`; derive `invitation`/`invitedEmail`/`returnTo`; change the post-auth `<Navigate to="/" replace/>` (`:15`) to `returnTo`; prefill (editable) `email: invitedEmail ?? ""` (`:11`); preserve `searchParams` on the register link (`:70`).
- [x] 6.7 GREEN (critical fix — primary invitee path is dead without this): `admin-web/src/app/views/RegisterPage.tsx:21-23` — change the unconditional `status === "anonymous"` redirect to `/login` into `if (status === "anonymous" && !invitation) return <Navigate to="/login" replace/>;` so an invitee with no account can reach the registration form.
- [x] 6.8 `RegisterPage.tsx` — add `useSearchParams`/`returnTo` (defaulting to `/setup/organization`); change the authenticated `<Navigate to="/" .../>` (`:18-19`) and the post-`signUp` `navigate("/setup/organization", ...)` (`:35`) to `returnTo`; prefill `email: invitedEmail ?? ""` (`:11`); when `invitation && invitedEmail` are both present render the email input `readOnly aria-readonly="true"` with the locked-field hint (`:52`); invitation-aware heading/copy (`:47-49`); preserve `searchParams` on the login link (`:59`).
...
- [x] 7.2 `cd admin-web && npm test` — full frontend suite green, including `InvitationAcceptPage.test.tsx`, `router.test.tsx`, and updated `LoginPage`/`RegisterPage` coverage. Confirmed: 9 test files, 49 tests, all passing (see apply-progress for exact evidence).
- [ ] 7.3 Manual: `docker compose up`, create an invitation, open the received Mailpit message at `127.0.0.1:8025`, follow the link with no existing account, register, and land as a member — confirms 6.7's fix and the full link contract end to end. **Not executable in this sandbox** (no Docker/running stack available to this agent) — left for the orchestrator/user to run manually against a live compose stack before this PR is considered fully verified end-to-end.
...
- [x] 8.2 Before requesting review, record the chain-strategy decision from the forecast above (`size:exception` for a single PR, or split per the Suggested Work Units) so `sdd-apply` does not proceed on an unresolved budget risk. Recorded for PR 2: the resolved chain strategy is **feature-branch-chain** (already decided in this file's Review Workload Forecast, not re-litigated here) — PR 2 (`feat/invitation-email-delivery-frontend`) targets PR 1's branch (`feat/invitation-email-delivery-backend`), stays scoped to Phases 5, 6, 7.2 (frontend only), and does not touch `backend/`. See `apply-progress.md` for the full record.

## Phase 9: Invitation Resend (out-of-band addition, post Phase 8)

> Added after the original tasks/design were written and reviewed. `CreateInvitationTx` correctly blocks a second pending invitation to the same email/org (`ErrInvitationPendingExists`), but that meant every manual SMTP test run needed a brand-new email address. This phase adds a "resend" action so the same pending invitation can be re-sent without inventing a new email each time. No formal spec/design cycle was run for this — see `apply-progress.md` for the out-of-band rationale.

- [x] 9.1 RED: `backend/internal/organizations/service_integration_test.go` — `TestResendInvitationRefreshesExpiryButKeepsToken` (expiry refreshed into the future, token unchanged, inviter context reflects the resending admin), `TestResendInvitationRejectsNonPending` (already-accepted invitation), `TestResendInvitationRejectsNonAdmin` (member requester), `TestResendInvitationNotFoundForWrongOrganization` (cross-tenant invitation id).
- [x] 9.2 RED: `backend/internal/organizations/handler_test.go` — `TestResendInvitationRouteRefreshesExpiryAndNotifiesOnce` (DB-backed, counting notifier, asserts exactly one resend notify call plus the one from creation), `TestResendInvitationRouteRejectsNonPending`, `TestResendInvitationRouteRequiresAdmin`; added an `ErrInvitationNotPending` case to `TestWriteOrganizationErrorMapsInvitationSafetyErrors`; added `ResendInvitation` to `organizationsRouteStub`.
- [x] 9.3 GREEN: `backend/internal/organizations/service.go` — `Service.ResendInvitation(ctx, requesterUserID, organizationID, invitationID) (InvitationCreation, error)`: `requireOrganizationAdmin`, `SELECT status ... FOR UPDATE` scoped to `id = $1 AND organization_id = $2` (not-found-shaped `ErrNotFound` if missing/cross-tenant), `ErrInvitationNotPending` if not `'pending'`, `UPDATE ... SET expires_at = now+invitationTTL, updated_at = NOW() RETURNING ...` (token untouched), re-reads org name + the *resending* admin's identity, single begin/commit transaction (no `ExecutePrepared`/idempotency — resending twice is at most a duplicate email, not a data-integrity issue, per the user's explicit scope note).
- [x] 9.4 GREEN: `backend/internal/organizations/handler.go` — `routeService.ResendInvitation` added to the interface; `POST /organizations/{organizationId}/invitations/{invitationId}/resend` registered; on success responds `200` with `pendingInvitationView(resent.Invitation)` (same shape as `ListInvitations`'s `PendingInvitation`, not the full `Invitation`/token-bearing creation shape); best-effort post-response notify via the existing `notifier.NotifyInvitation` (flush-then-notify, same pattern as creation's non-idempotent branch, error swallowed/logged exactly like creation); `writeOrganizationError` gained an `ErrInvitationNotPending` → `400` case (reused the existing dual `errors.Is`/`err.Error()` matching pattern already used for the other invitation errors in this switch).
- [x] 9.5 `admin-web/src/lib/api/organizations.ts` — `resendOrganizationInvitation(token, organizationId, invitationId)`, kept in this module (not `invitations.ts`) since it owns the other org-admin invitation calls (`createOrganizationInvitation`, `listOrganizationInvitations`); reuses the existing `normalizeInvitation`/`RawInvitation` mapping.
- [x] 9.6 `admin-web/src/features/members/mutations.ts` — `useResendInvitationMutation(token, organizationId)`, structured like `useInviteMemberMutation` (invalidates the invitations query key on success); no idempotency-key/`useUncertainCreationKey` wiring since the endpoint doesn't use one.
- [x] 9.7 RED: `admin-web/src/features/members/MembersPage.test.tsx` — `"resends a pending invitation and shows a confirmation notice"` (asserts the resend POST fires exactly once and an "Invitation resent" notice appears), `"shows an error notice when resending an invitation fails"` (400 response renders a "Resend failed" notice). Confirmed RED against pre-change `MembersPage.tsx` (no Resend button, both tests failed on the missing `button` role).
- [x] 9.8 GREEN: `admin-web/src/features/members/MembersPage.tsx` — added an "Actions" column to the pending-invitations table with a per-row "Resend"/"Resending…" button (`aria-label="Resend invitation to {email}"`), wired to `useResendInvitationMutation`, reusing the existing `notice` state pattern (`tone: "neutral"` success / `tone: "danger"` failure) already used by the invite-member flow.
- [x] 9.9 `cd backend && go test ./... -p 1 -parallel 1` against a real ephemeral Postgres — all 12 packages pass, including the 7 new resend tests. `cd admin-web && npm test -- --run` — 9 files, 51 tests, all pass (49 pre-existing + 2 new). `npx tsc --noEmit` clean. `gofmt -l`/`go vet ./...` clean.
