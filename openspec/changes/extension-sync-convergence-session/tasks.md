...
| Field | Value |
|---|---|
| Remaining authored lines (tests + artifact update included) | 2,420–2,750 |
| Per-unit budget / correction reserve | 180–200 / 200–220 (PR4a0a, complete); 180–240 / >=100 (PR4a0a2); 220 / 150 (PR4a0a3a.1); 240 / 140 (PR4a0a3a.2); 220–300 / >=100 (PR4a0a3b); 250–300 / 100–150 (PR4a0b); 330–380 / 20–70 thereafter |
| Delivery / chain | auto-chain / stacked-to-main; target `develop` in order |
| Suggested chain | PR4a0a → PR4a0a2 → PR4a0a3a.1 → PR4a0a3a.2 → PR4a0a3b → PR4a0b → PR4a1 → PR4a2 → PR4a3 → PR4b |
...
| PR4a0a3a.2 canonical folder/bookmark prepared adapters (240 / 140) | PR4a0a3a.1 → immutable prepared folder/bookmark adapters; exclude kernel concurrency proof, apply/mutation, routes, publisher, HTTP idempotency, migration | `backend/internal/bookmarks/{service.go,service_integration_test.go}`, `backend/internal/sync/{types.go,service.go,postgres.go}`. RED: auth/containment/ancestry, trim/validate/clamp/full-shape/fingerprint/no-op, legacy `Update*`. `cd backend && go test ./internal/bookmarks ./internal/sync`; PostgreSQL `postgres:5432` adapter harness. | Adapters/tests only. |
