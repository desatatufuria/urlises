...
| Field | Value |
|---|---|
| Remaining authored lines (tests + artifact update included) | 2,310–2,570 |
| Per-unit budget / correction reserve | 180–200 / 200–220 (PR4a0a, complete); 180–240 / >=100 (PR4a0a2); 280–350 / >=50 (PR4a0a3a); 220–300 / >=100 (PR4a0a3b); 250–300 / 100–150 (PR4a0b); 330–380 / 20–70 thereafter |
| Delivery / chain | auto-chain / stacked-to-main; target `develop` in order |
| Suggested chain | PR4a0a → PR4a0a2 → PR4a0a3a → PR4a0a3b → PR4a0b → PR4a1 → PR4a2 → PR4a3 → PR4b |
...
Progress: 32/46 semantic tasks complete. Native checklist progress is 15/29 because legacy grouped checkboxes represent multiple semantic tasks; splitting two unchecked PR4a0a3 checkboxes into four changes the denominator only.
...
| PR4a0a3a prepare foundation (280–350 / >=50) | PR4a0a2 → unused immutable locked preparation; exclude resource/order/event/cursor mutation, routes, publisher, HTTP idempotency, migration | `backend/internal/bookmarks/{service.go,service_integration_test.go}`, `backend/internal/sync/{types.go,service.go,postgres.go,postgres_integration_test.go}`. RED: read-only tentative source/destination scopes; globally sorted/deduplicated advisory locks before target/sibling rows; locked revalidation and bounded whole-tx drift retry/refusal; auth/containment/ancestry, canonical no-op. `cd backend && go test ./internal/bookmarks ./internal/sync`; PostgreSQL deterministic blocking/opposite/same-scope/drift harness, no deadlock or writes. | Remove only prepare API, lock/normalization helpers, and tests; retain legacy `Update*` and PR4a0a2. |
| PR4a0a3b apply foundation (220–300 / >=100) | PR4a0a3a → unused same-tx prepared apply; exclude prepare/lock changes, routes, HTTP idempotency, migration, publisher invocation | `backend/internal/bookmarks/{service.go,service_integration_test.go}`, `backend/internal/sync/{types.go,service.go,postgres.go,postgres_integration_test.go}`. RED: zero-write no-op, exact prepared mutation/one event-cursor, rollback, legacy compatibility. `cd backend && go test ./internal/bookmarks ./internal/sync`; PostgreSQL transaction harness. | Remove only apply API/tests; retain unused prepare foundation and legacy `Update*`. |
| PR4a0b complete-shape integration (250–300 / 100–150) | PR4a0a2 + PR4a0a3a + PR4a0a3b → folder/bookmark route wiring and post-commit invocation; exclude extension | `backend/internal/sync/{types,service,postgres,bookmark_routes,headers}.go`, `{bookmark_routes,postgres}_test.go`. RED: partial composition, replay/conflict, later-event-stable ack, no event/cursor advance, real mutation one event/cursor with auth/base-cursor/containment. `cd backend && go test ./internal/sync ./internal/bookmarks`; HTTP/PostgreSQL route harness proves no-op and mutation. | PATCH route wiring/tests; revert to unused prepared foundations, no route event semantics. |
...
## Phase 13b: Prepare foundation — PR4a0a3a
- [ ] 13b.1 RED: in `backend/internal/{bookmarks/service_integration_test.go,sync/postgres_integration_test.go}`, prove external-`pgx.Tx` `PrepareFolderPatchTx`/`PrepareBookmarkPatchTx`: read-only tentative source/destination scopes; globally sorted/deduplicated advisory locks before target or sibling `FOR UPDATE`; locked target/scope rederivation, bounded whole-prepared-transaction retry/refusal on drift, and no post-row-lock scope acquisition; auth/containment/ancestry, trim/validate/clamp/normalize, fingerprint/final-shape/no-op. PostgreSQL: opposite moves, same-scope move, concurrent drift, deterministic blocking/no deadlock, zero resource/order/event/cursor writes, legacy `Update*` compatibility. Run `cd backend && go test ./internal/bookmarks ./internal/sync`; rollback: remove only these RED tests.
- [ ] 13b.2 GREEN: in `backend/internal/bookmarks/service.go` and `backend/internal/sync/{types.go,service.go,postgres.go}`, add immutable `PreparedPatch` and external-tx prepare methods that discover scopes read-only, acquire globally sorted/deduplicated advisory locks before target/sibling row locks, lock target and siblings `ORDER BY position,id`, then rederive/revalidate locked scopes; on drift return retryable refusal for bounded whole-transaction retry and never acquire a scope after row locking. Revalidate auth/containment/ancestry and canonicalize before fingerprinting; preserve zero resource/order/event/cursor writes and legacy `Update*`. Run the same command; rollback: remove only prepare API/helpers/tests, retaining legacy `Update*` and PR4a0a2.

## Phase 13c: Apply foundation — PR4a0a3b
...
- [ ] 14.1 RED: after PR4a0a2 + PR4a0a3a + PR4a0a3b, prove partial shape, replay/conflict, stable ack, no advance, and real mutation regression.
