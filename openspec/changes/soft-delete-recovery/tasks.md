...
- [ ] 5.1 `cd backend && go build ./... && go vet ./... && go test ./internal/organizations ./internal/workspaces ./internal/groups ./internal/access ./internal/activity ./internal/purge ./internal/sync` — build/vet clean, all touched packages pass.
- [ ] 5.2 `cd admin-web && npm run build && npm test -- WorkspacesPage TrashPage format ConfirmByTyping OrganizationSetupPage` — build clean, all touched suites pass.
- [ ] 5.3 Check current Docker/Postgres availability before relying on it (no assumption either way carried over from planning). If available: `docker compose up`; as an org admin, delete a workspace via typed-name confirm and delete an organization, confirm both vanish from listings/sync/websocket immediately, restore each from Trash, confirm full access returns, back-date a `deleted_at` row past 30 days and confirm the next sweep purges it. If unavailable this session, defer and note the fallback (production validation), matching `lifecycle-management`'s task 6.3 precedent — do not assume success.
